...
case class Polygon(val cList: List[(Double, Double)]) extends Figure {
    ...
    override def getBoundingBox: (Double, Double, Double, Double ) = {
        val xmin = cList.map(x=>x._1).min
        val xmax = cList.map(x=>x._1).max
        val ymin = cList.map(y=>y._2).min
        val ymax = cList.map(y=>y._2).max
        (xmin, xmax, ymin,ymax)
    }
    ...
    override def translate(shiftX: Double, shiftY: Double): Polygon = {
        Polygon(cList.map(s => (s._1 + shiftX, s._2 + shiftY)))
    }
